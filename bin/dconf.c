...
static gboolean
option_error_from (GError *error)
{
  g_assert (error != NULL);

  error->domain = G_OPTION_ERROR;
            ...
            GError      **error)
{
  ...
  key = argv[index];
  if (!dconf_is_key (key, error))
    return option_error_from (*error);
...
static void
show_path (DConfClient *client, gchar *path)
{
             ...
             GError     **error)
{
  ...
  g_autoptr(GVariantType) reply_type = NULL;
                      ...
                      GError      **error)
{
  ...
  files = list_directory (dirname, S_IFREG, error);
  if (files == NULL)
    {
  ...
  g_ptr_array_sort (files, string_compare);
              ...
              gpointer     user_data)
{
  ...
  /* We process the files in reverse alphabetical order. If the key is already
   * set then it must have been set from a file with higher precedence so we
  ...
  for (guint i = 0; i != files->len; ++i)
    {
      ...
      if (!update_directory (name, error))
        {
  ...
  if (failed)
    {
      g_set_error_literal (error, DCONF_ERROR, DCONF_ERROR_FAILED,
                           "failed to update one of databases");
      return FALSE;
    }
                   ...
                   FILE     *file)
{
  ...
  if (cmd == NULL)
    ...
    {
      /* Generate command specifig usage help text. */
